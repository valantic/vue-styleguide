...
- [docs] Add `docs/guide/demo-page-patterns.md` documenting the `c-vas-demo-card` vs. `l-vas-layout` vs. `c-vas-demo-table` decision rule for demo pages, and document `c-vas-demo-table` for the first time; cross-link it from the demo-card and custom-settings guides.
- [feat] Add `blueprints/r-component-demo.vue` and `blueprints/r-page-demo.vue`, and a Claude Code skill (`ai/skills/vue-styleguide-demo-page`) consumers can copy into their own `.claude/skills/` to generate or restructure demo pages following the documented pattern.
- [refactor] Restructure the `r-icon` and `r-vas-modal` demo pages to use `c-vas-demo-card`, aligning them with the documented small-component pattern.
- [feat] Add an "AI" section (new `i-ai` icon) to the top of the sidebar's right icon bar, introducing the `vue-styleguide-demo-page` Claude Code skill and linking to its setup guide.
- [build] Add `ai` to the npm package's `files`, so the `vue-styleguide-demo-page` skill ships with the package and can be symlinked into a consumer's `.claude/skills/`.
- [docs] Add `docs/guide/ai-skill.md` documenting how to point Claude Code at the shipped skill (symlink into `.claude/skills/`, or reference it from `CLAUDE.md`); link it from the sidebar interface guide and demo page patterns guide.
- [fix] Fix links inside the sidebar being unreadable on the dark theme (rendering in a fixed dark color instead of the theme's text color). Root cause was two-fold: `c-vas-sidebar`'s own CSS reset unsets `<a>` color via a zero-specificity `:where()` selector, which a project's own plain `a { color: … }` rule can always beat regardless of load order — `c-vas-sidebar` now also sets `a { color: inherit; }` with real specificity so it wins by default; and this repo's own demo-app base styles (`_basics.scss`) applied a fixed, non-theme-reactive link color via an ID selector that unintentionally leaked into the sidebar overlay, now excluded via `a:not(.c-vas-sidebar, .c-vas-sidebar *)`. Components with an intentional accent link color (e.g. `c-vas-x-ray-mode`, `c-vas-ai`) keep using `!important` to override the new default.
