...
const collectAncestorElements = (el: Element | null): Element[] => {
  return el ? [el, ...collectAncestorElements(el.parentElement)] : [];
};

export const findComponentInstance = (
  el: Element | null,
): { instance: VueInternalInstance; el: Element } | null => {
...
const collectStackedInstanceChain = (instance: VueInternalInstance, el: Element): ComponentStack => {
...
export const collectStackedInstances = (instance: VueInternalInstance, el: Element): ComponentStack => {
  return collectStackedInstanceChain(instance, el);
};

export const getComponentDisplayName = (instance: VueInternalInstance): string | null => {
  // eslint-disable-next-line no-underscore-dangle
...
// see formatCopyText().
export const toProjectRelativePath = (file: string | null | undefined): string | null => {
  if (!file) {
    return null;
  }
...
export const getComponentFilePath = (instance: VueInternalInstance): string | null => {
  // eslint-disable-next-line no-underscore-dangle
...
const pickPreferredInstance = (stack: ComponentStack): VueInternalInstance => {
...
export const resolveComponentAtElement = (el: Element | null): ResolvedComponent | null => {
...
export const formatCopyText = (resolved: ResolvedComponent): string => {
...
export const markComponentInstance = (el: unknown, name: string | null, file: string | null): void => {
...
export const hasComponentMarker = (el: unknown): boolean => {
  return el instanceof Element && Boolean((el as MarkedElement).vasComponents?.length);
};

const pickPreferredMarkerEntry = (entries: [MarkerEntry, ...MarkerEntry[]]): MarkerEntry => {
...
const resolveMarkedElement = (el: Element, entries: [MarkerEntry, ...MarkerEntry[]]): ResolvedComponent => {
...
export const getComponentBreadcrumb = (el: Element | null): ResolvedComponent[] => {
